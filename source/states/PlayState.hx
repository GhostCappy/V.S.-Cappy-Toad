...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'arcade': new states.stages.Arcade(); // Arcade
			case 'giobar': new states.stages.GioBar(); // Gio's Bar
			case 'blank': new states.stages.Blank(); // Blank
			case 'castletop': new states.stages.CastleTop(); // Castle Top
			case 'meadowraised': new states.stages.MeadowRaised(); // Meadow Raised
			case 'room': new states.stages.Room(); // Room
			case 'space': new states.stages.Space(); // Space
			case 'valley': new states.stages.Valley(); // Valley (OLD)
			case 'valleynight': new states.stages.ValleyNight(); // Valley Night
			case 'newvalley': new states.stages.NewValley(); // Valley (NEW)
		}
		...
		for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), 'scripts/'))
			#if linux
			for (file in CoolUtil.sortAlphabetically(FileSystem.readDirectory(folder)))
			#else
	...
	public function endSong()
	{
		...
		checkForAchievement([weekNoMiss, 'ur_bad', 'ur_good', 'hype']);
		#end
   ...
   */
   function zoomIntoResultsScreen(isNewHighscore:Bool,scoreData:SaveScoreData,prevScoreRank:ScoringRank):Void
	...
	function moveToResultsScreen(isNewHighscore:Bool,scoreData:SaveScoreData,prevScoreRank:ScoringRank):Void
	{
	  ...
	  var modName = "unknown";
	  trace(songName);
	  if (songName == 'capi-mixed' || songName == 'fast-mixed' || songName == 'vocal-point-mixed' || songName == 'ice-night-mixed' || songName == 'caturn-mixed') 
	  {
		modName = "GhostCappy and RedRetroThunder";
	  }
	  else
	  {
		modName = "GhostCappy";
	  ...
	  var res:ResultState = new ResultState(
		{
		  ...
		  songId: curSong,
		  difficultyId: Difficulty.getString(),
		  title: isStoryMode ? ('${storyCampaignTitle}') : ('${curSong} by ${modName}'),
		  scoreData:scoreData,
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null)
	{
		...
		for (name in achievesToCheck) {
			...
			if (name != WeekData.getWeekFileName() + '_nomiss') // common achievements
			{
				...
				{
					case 'ur_bad':
						unlock = (ratingPercent < 0.5 && !practiceMode);
